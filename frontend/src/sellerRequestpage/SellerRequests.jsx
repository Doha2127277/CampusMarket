                        ...
                        <div key={item.id} className="seller-horizontal-card" style={{ 
                            ...
                            <div style={{ width: '200px', minWidth: '200px', backgroundColor: '#f9f9f9', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRight: '1px solid #eee' }}>
                                {finalImage ? (
                                    ...
                                    <div style={{ textAlign: 'center', color: '#ccc' }}><p style={{ fontSize: '12px' }}>No Image</p></div>
                                )}
                            </div>

                            {/* معلومات الطلب */}
                            <div style={{ flex: 1, padding: '20px', display: 'flex', flexDirection: 'column' }}>
                                <div className={`status-badge-web ${item.status}`} style={{ width: 'fit-content' }}>
                                    {item.status ? item.status.replace(/_/g, ' ') : 'Pending'}
                                </div>
                                <h3 style={{ margin: '15px 0 5px 0', color: '#1a1a1a' }}>{item.productName || (item.items && item.items[0]?.name) || "Product"}</h3>
                                ...
                                <div style={{ marginTop: 'auto' }}>
                                    {activeTab === 'donations' && item.status === 'approved_by_admin' && (
                                        <button className="approve-btn-web" style={{ backgroundColor: '#10b981', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: '8px', cursor: 'pointer' }} onClick={() => handleDonorApprove(item.id)}>
                                            Confirm Giving
                                        </button>
                                    )}
                                </div>
                            ...
                            <div style={{ flex: 1.2, padding: '15px', borderLeft: '1px solid #eee', display: 'flex', flexDirection: 'column', backgroundColor: '#fcfcfc' }}>
                                ...
                                <div className="chat-input-web" style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
                                    ...
                                    />
                                    <button style={{ borderRadius: '20px', padding: '8px 15px', cursor: 'pointer' }} onClick={() => handleAddComment(item.id, activeTab === 'sales' ? "orders" : "volunteer_requests")}>
                                        Send
                                    </button>
                                </div>
                            </div>
